    ...
    def print_bus(self, *messages):
        ...
        for msg in messages:  # Append bus messages to the output
            if isinstance(msg, str):  # If the message is a string, leave it as it is
                msg_str = msg
            elif isinstance(msg, (int, float)):  # Check if the message is a numeric type
                msg_str = "{0:.4g}".format(msg)  # Convert to string with 4 significant figures
                if msg >= 0:  # Append a space before the value if it is not negative
                    msg_str = " " + msg_str
            elif isinstance(msg, list):  # If the message is a list
                msg_str = ', '.join(str("{0:.4g}".format(item)) for item in msg if isinstance(item, (int, float)))
                # Format each numeric item in the list
            else:  # For other types, convert directly to string without formatting
                msg_str = str(msg)

            output_string += " " + msg_str  # Append the message string to the output string
